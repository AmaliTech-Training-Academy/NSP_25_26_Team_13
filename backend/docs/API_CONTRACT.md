...
- `Authorization: Bearer JWT_TOKEN_PLACEHOLDER` is not required by current config, but may still be used by clients for future
  compatibility.
...
- `Content-Type: application/json`
- `Authorization: Bearer JWT_TOKEN_PLACEHOLDER` (Optional)
